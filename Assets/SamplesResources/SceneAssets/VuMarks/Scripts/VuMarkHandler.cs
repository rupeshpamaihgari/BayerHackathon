...
public class VuMarkHandler : MonoBehaviour
{
    ...
    public Dropdown modeSelectorDropdown;
    public Dropdown CropTypeDropdown;
    public int mode = 0;
    ...
    const int PERSISTENT_NUMBER_OF_CHILDREN = 1;
    ...
    void OnVuMarkFound(VuMarkBehaviour vuMarkBehaviour)
    {
        ...
        //if (vuMarkBehaviour.transform.childCount > 1)
        //{
         //   for (int i = 1; i < vuMarkBehaviour.transform.childCount; i++)
        //    {
        //        vuMarkBehaviour.transform.GetChild(i).gameObject.SetActive(true);
        //    }
       // }
    ...
    void OnVuMarkLost(VuMarkBehaviour vuMarkBehaviour)
    {
        ...
        //for(int i = 1; i < vuMarkBehaviour.transform.childCount; i++)
        //{
        //    vuMarkBehaviour.transform.GetChild(i).gameObject.SetActive(false);
        //}
    ...
    void SetVuMarkAugmentation(VuMarkBehaviour vuMarkBehaviour)
    {
        ...
        Vector3 initiationPoint = new Vector3(vuMarkBehaviour.transform.position.x, vuMarkBehaviour.transform.position.y, vuMarkBehaviour.transform.position.z+1.5f);

        GameObject Augmentation = Instantiate(sourceAugmentation, vuMarkBehaviour.transform);
        Augmentation.transform.position = initiationPoint;

        if (mode == 0)
        {
            Augmentation.transform.GetChild(0).gameObject.SetActive(true);
        }
        else if (mode == 1)
        {
            Augmentation.transform.GetChild(1).gameObject.SetActive(true);
        }
        else if (mode == 2)
        {
            if(cropType==0)
                Augmentation.transform.GetChild(2).gameObject.SetActive(true);
            else if (cropType == 1)
                Augmentation.transform.GetChild(3).gameObject.SetActive(true);
        }
    ...
    {
        if (modeSelectorDropdown.value == 0)
        {
            mode = 0;
            CropTypeDropdown.gameObject.SetActive(false);
        }
        else if(modeSelectorDropdown.value == 1)
        {
            mode = 1;
            CropTypeDropdown.gameObject.SetActive(false);
        }
        else if(modeSelectorDropdown.value == 2)
        {
            mode = 2;
            CropTypeDropdown.gameObject.SetActive(true);
        }
    }

    public void CropTypeChanged()
    {
        if (CropTypeDropdown.value == 0)
        {
            cropType = 0;
        }
        else if (CropTypeDropdown.value==1)
        {
            cropType = 1;
        }
    }
    ...
    public void LoadHealthInspection()
    {
